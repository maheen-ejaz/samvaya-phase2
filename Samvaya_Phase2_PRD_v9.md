...
#### `bgv_checks`
Tracks the 13 background verification checks per applicant. Each check has its own row.
...
Background verification is conducted via a third-party provider, covering 13 checks across identity, financial, professional, legal, address, and digital footprint categories.
...
| In Progress | 🔄 | Check submitted, awaiting result |
...
- Upload supporting document or report from the BGV provider
...
**Verification Tracker** (for all fee-paying members):
...
- Upload field per check for BGV reports/documents
...
**Trigger:** Background verification results come in and the team marks verification as complete in the admin dashboard.
...
Background verification is conducted by a third-party provider and costs are incurred immediately upon initiation. No refunds will be issued once verification has begun, regardless of outcome or circumstances.
...
| 4 | BGV provider and services | ✅ Third-party provider — 13 checks defined in Section 4B. Triggered by verification fee payment + consent. |
...
| BGV provider | Third-party provider | Covers all 13 required checks. Cost passed through to applicant at ₹6,000 + GST. |
