...
| Fact | Value |
|------|-------|
| Form | 100 base questions, 13 sections (A–M), 3 Claude chats (Q38, Q75, Q100 in Section M) |
| Verification fee | ₹6,000 + 18% GST = **₹7,080** |
...
|-----|-------|----------------|
| Verification | **₹7,080** (₹6K + 18% GST) | After form submitted + payment confirmed |
...
- 100 base questions, 13 sections (A–M), ~28–32 conditional questions
- 3 Claude AI chats in Section M (all grouped at the end, after all factual sections A–L):

| Chat | Question ID | questionNumber | Exchanges |
|------|-------------|----------------|-----------|
| Conv 1 — Family | Q38 | 101 | 4 max |
| Conv 2 — Goals & values | Q75 | 102 | 6 max |
| Conv 3 — Closing | Q100 | 103 | 1 only |
...
- Confidentiality callouts at Sections C, E, F, K (see PRD 4.2 for exact copy)
- **Temporary testing state — MUST restore before launch:** All fields optional (`required: false`), `minFiles = 0`, chat skip buttons enabled
...
|------|------------------------|
| Pricing is ₹7,080 and ₹41,300 | Silently corrupts legal copy, emails, and DB |
