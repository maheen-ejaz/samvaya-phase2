...
- Every member undergoes a 13-point background verification (OnGrid)
...
| BGV | OnGrid | 13 checks per applicant |
...
No refunds once verification has begun (costs incurred with OnGrid immediately). No refunds on membership fee (payment only requested after mutual interest). Exception: technical error causing duplicate/incorrect charge — must be raised within 7 days.
...
**OnGrid** — 13 checks per applicant across identity, financial, professional, legal, address, and digital footprint.
