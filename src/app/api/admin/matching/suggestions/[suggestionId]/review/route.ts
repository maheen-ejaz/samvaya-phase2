...
import { generateMatchToken } from '@/lib/share/match-token';
...
export async function POST(
  ...
  try {
    ...
    if (action === 'approve') {
      ...
      // Generate share token (fire-and-forget)
      const pres = presentation as Record<string, unknown>;
      generateMatchToken(supabase, pres.id as string, pres.expires_at as string).catch((err) => {
        console.error('Failed to generate match share token:', err);
      });
